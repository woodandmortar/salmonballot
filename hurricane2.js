...
// Array to hold the spheres
const spheres = [];

// Create 10 spheres and add them to the scene
for (let i = 0; i < 10; i++) {
    const geometry = new THREE.SphereGeometry(5, 32, 32);
    const material = new THREE.MeshBasicMaterial({ color: Math.random() * 0xffffff });
    const sphere = new THREE.Mesh(geometry, material);
    sphere.position.set(Math.random() * 200 - 100, Math.random() * 200 - 100, Math.random() * 200 - 100); // random initial positions
    scene.add(sphere);
    spheres.push(sphere);
}
...
renderer.domElement.addEventListener('click', function(event) {
    ...
    // Move spheres to surround the clicked point but constrain them within 820 units from the origin
    for (let i = 0; i < spheres.length; i++) {
        const offset = new THREE.Vector3(Math.random() * 50 - 25, Math.random() * 50 - 25, Math.random() * 50 - 25); // random offsets
        let targetPos = new THREE.Vector3(pos.x + offset.x, pos.y + offset.y, pos.z + offset.z);

        // Ensure the sphere is no more than 820 units away from the origin
        targetPos.normalize().multiplyScalar(820);
        spheres[i].position.copy(targetPos);
    }
});

animate2();
function updatePositionsDisplay(spheres) {
    const container = document.getElementById("positionsContainer");
    let content = "";

    spheres.forEach((sphere, index) => {
        const position = sphere.position;
        content += `<p>Sphere ${index + 1}: X: ${position.x.toFixed(2)}, Y: ${position.y.toFixed(2)}, Z: ${position.z.toFixed(2)}</p>`;
    });

    container.innerHTML = content;
}

// Assuming "spheres" is an array containing all your spheres
updatePositionsDisplay(spheres);
